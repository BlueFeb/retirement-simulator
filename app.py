...
PRESETS = {
    "30대 직장인": dict(mode="detailed",age=32,gender="male",retire_age=60,life_expectancy=85,
        inflation_rate=2.5,deposit_amount=2000,deposit_rate=DEP_RATE,stock_amount=3000,stock_return=7.0,
        real_estate_amount=0,real_estate_return=3.0,other_assets=0,salary=350,side_income=0,
        pension_monthly=60,pension_start_age=65,fixed_cost=80,variable_cost=70,savings_rate=35,loans=[]),
    "40대 맞벌이": dict(mode="detailed",age=42,gender="male",retire_age=58,life_expectancy=85,
        inflation_rate=2.5,deposit_amount=5000,deposit_rate=DEP_RATE,stock_amount=8000,stock_return=7.0,
        real_estate_amount=50000,real_estate_return=3.0,other_assets=1000,salary=600,side_income=50,
        pension_monthly=100,pension_start_age=65,fixed_cost=180,variable_cost=120,savings_rate=25,
        loans=[{"amount":30000,"rate":LOAN_RATE,"years":25}]),
    "50대 은퇴준비": dict(mode="detailed",age=52,gender="male",retire_age=60,life_expectancy=88,
        inflation_rate=2.5,deposit_amount=10000,deposit_rate=DEP_RATE,stock_amount=15000,stock_return=5.0,
        real_estate_amount=60000,real_estate_return=2.0,other_assets=5000,salary=500,side_income=0,
        pension_monthly=120,pension_start_age=65,fixed_cost=150,variable_cost=100,savings_rate=30,
        loans=[{"amount":10000,"rate":3.5,"years":8}]),
}
...
def main():
    ...
    # Google Sheets 상태 (디버그 포함)
    gsheet_ok = get_gsheet_connection() is not None
    if gsheet_ok:
        st.caption("✅ Google Sheets 연동 — 분석 시 자동 저장")
    else:
    ...
    # URL 복원
    qp = st.query_params
    restored = decode_params(qp.get("d","")) if "d" in qp else None
    ...
    with st.expander("⚡ 빠른 시작 — 프리셋"):
        ...
        for i,(name,preset) in enumerate(PRESETS.items()):
            if cols[i].button(name, key=f"preset_{i}", use_container_width=True):
                for k,v in preset.items(): st.session_state[f"p_{k}"] = v
                st.rerun()

    mode = st.radio("분석 모드", ["간단 분석","상세 분석"], horizontal=True,
                    label_visibility="collapsed",
                    index=1 if (restored and restored.get("mode")=="detailed") else 0)
    ...
    inflation_rate = st.number_input("연간 물가상승률 (%)", 0.0, 15.0, float(_d.get("inflation_rate",2.5)), 0.1,
    ...
    if is_simple:
        st.subheader("💰 자산 & 수입")
        total_savings = amt("총 모아놓은 자금 (만원)","ts",_d.get("total_savings",10000),1000,"예금+주식+부동산 등")
        monthly_income = amt_s("월 수입 — 세후 (만원)","mi",_d.get("monthly_income",400))
        st.subheader("💸 지출")
        monthly_expense = amt_s("월 평균 지출 (만원)","me",_d.get("monthly_expense",250))
        ...
        st.subheader("🏦 보유 자산")
        st.markdown("**예금·적금**")
        deposit_amount = amt("예금 (만원)","da",_d.get("deposit_amount",3000),500,f"평균 금리 {DEP_RATE}%")
        deposit_rate = st.number_input("예금 수익률 (%)",0.0,20.0,float(_d.get("deposit_rate",DEP_RATE)),0.1)
        st.markdown("**주식·펀드·ETF**")
        stock_amount = amt("주식 (만원)","sa",_d.get("stock_amount",4000),500)
        stock_return = st.number_input("기대 수익률 (%)",-20.0,30.0,float(_d.get("stock_return",7.0)),0.5)
        st.markdown("**부동산**")
        real_estate = amt("부동산 시가 (만원)","rea",_d.get("real_estate_amount",30000),5000)
        re_return = st.number_input("부동산 상승률 (%)",-10.0,20.0,float(_d.get("real_estate_return",3.0)),0.5)
        ...
        pension_start = st.number_input("연금 시작 나이",55,80,_d.get("pension_start_age",65))
        st.markdown("<hr class='section-divider'>", unsafe_allow_html=True)
        st.subheader("💸 지출")
        ...
        savings_rate = st.number_input("저축률 (%)",0.0,100.0,float(_d.get("savings_rate",30)),5.0)
        st.markdown("<hr class='section-divider'>", unsafe_allow_html=True)
        st.subheader("🏠 대출")
    ...
    if st.button("🚀 시뮬레이션 실행", use_container_width=True, type="primary"):
        ...
        st.plotly_chart(chart_scenarios(base,s2,s3,retire_age,dark),use_container_width=True,config=pcfg)
            ...
            with st.expander("📈 물가상승률 영향 상세"):
                ...
                if is_simple:
                    ...
                    st.markdown(f"| 시점 | 월 지출 |\n|---|---:|\n| 현재 | **{ne:,.0f}**만원 |\n"
                                f"| 은퇴({retire_age}세) | **{ne*((1+inflation_rate/100)**el_ret):,.0f}**만원 |\n"
                                f"| {life_exp}세 | **{ne*((1+inflation_rate/100)**el_end):,.0f}**만원 |")
                else:
        ...
        # PDF — matplotlib 기반 (kaleido 불필요)
        with st.spinner("PDF 보고서 생성 중 (차트 포함)..."):
            try:
                chart_imgs = generate_chart_images_for_pdf(
                    df, params, dep_info, retire_age,
                ...
                st.download_button("📄 PDF 보고서 다운로드 (차트 포함)", pdf,
