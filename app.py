...
# ── 헬퍼 ──
def _sv(key, default):
    """session_state에서 값 가져오기 (프리셋/URL 복원 지원)."""
    return st.session_state.get(key, default)

def amt(label, key, default, step=1000, help_text=None, max_val=10_000_000_000):
    val = st.number_input(label, 0, max_val, _sv(key, default), step=step, key=key, help=help_text)
...
def decode_params(encoded):
    try: return json.loads(base64.urlsafe_b64decode(encoded.encode()).decode())
    except: return None
...
PRESETS = {
    "30대 독신": dict(
        age=30, gender="male", retire_age=60, life_expectancy=85, inflation_rate=2.5,
        deposit_amount=1500, deposit_rate=DEP_RATE, stock_amount=2000, stock_return=7.0,
        real_estate_amount=0, real_estate_return=3.0, other_assets=0,
        salary=300, side_income=0, pension_monthly=50, pension_start_age=65,
        fixed_cost=60, variable_cost=60, savings_rate=40, num_loans=0, loans=[]),
    "40대 외벌이": dict(
        age=42, gender="male", retire_age=60, life_expectancy=85, inflation_rate=2.5,
        deposit_amount=5000, deposit_rate=DEP_RATE, stock_amount=5000, stock_return=6.0,
        real_estate_amount=40000, real_estate_return=2.5, other_assets=1000,
        salary=450, side_income=0, pension_monthly=90, pension_start_age=65,
        fixed_cost=150, variable_cost=100, savings_rate=20, num_loans=1,
        loans=[{"amount":25000, "rate":LOAN_RATE, "years":22}]),
    "40대 맞벌이": dict(
        age=42, gender="male", retire_age=58, life_expectancy=85, inflation_rate=2.5,
        deposit_amount=8000, deposit_rate=DEP_RATE, stock_amount=12000, stock_return=7.0,
        real_estate_amount=60000, real_estate_return=3.0, other_assets=2000,
        salary=700, side_income=50, pension_monthly=120, pension_start_age=65,
        fixed_cost=200, variable_cost=150, savings_rate=25, num_loans=1,
        loans=[{"amount":35000, "rate":LOAN_RATE, "years":25}]),
    "50대 노후준비": dict(
        age=52, gender="male", retire_age=60, life_expectancy=88, inflation_rate=2.5,
        deposit_amount=15000, deposit_rate=DEP_RATE, stock_amount=20000, stock_return=5.0,
        real_estate_amount=70000, real_estate_return=2.0, other_assets=5000,
        salary=500, side_income=0, pension_monthly=130, pension_start_age=65,
        fixed_cost=150, variable_cost=100, savings_rate=30, num_loans=1,
        loans=[{"amount":8000, "rate":3.5, "years":8}]),
}

def apply_preset(preset):
    """프리셋 값을 session_state의 입력 위젯 키에 직접 매핑."""
    key_map = {
        "age":"age", "gender":"gender_sel", "retire_age":"retire", "life_expectancy":"life",
        "inflation_rate":"infl",
        "deposit_amount":"da", "deposit_rate":"dr", "stock_amount":"sa", "stock_return":"sr",
        "real_estate_amount":"rea", "real_estate_return":"rr", "other_assets":"oa",
        "salary":"sal", "side_income":"si", "pension_monthly":"pen", "pension_start_age":"ps",
        "fixed_cost":"fc", "variable_cost":"vc", "savings_rate":"svr",
        "num_loans":"nl",
    }
    for pk, wk in key_map.items():
        if pk in preset:
            st.session_state[wk] = preset[pk]

    # 성별 변환
    if "gender" in preset:
        st.session_state["gender_sel"] = 0 if preset["gender"] == "male" else 1

    # 대출
    loans = preset.get("loans", [])
    st.session_state["nl"] = len(loans)
    for i, l in enumerate(loans):
        st.session_state[f"la{i}"] = l["amount"]
        st.session_state[f"lr{i}"] = l["rate"]
        st.session_state[f"ly{i}"] = l["years"]
...
def main():
    ...
    with hc2:
        if st.button("🌙 야간" if not dark else "☀️ 주간", key="theme_toggle", use_container_width=True):
    ...
    if gsheet_ok:
        ...
        st.caption(f"⚠️ Sheets 미연결 — {err}" if err else "⚠️ Sheets 미연결 — README 참고")

    # URL 복원
    qp = st.query_params
    if "d" in qp and "url_restored" not in st.session_state:
        restored = decode_params(qp.get("d",""))
        if restored:
    ...
    mode = st.radio("분석 모드", ["간단 분석","상세 분석"], horizontal=True,
                    label_visibility="collapsed", index=mode_idx, key="mode_radio")
    ...
    # ━━ 기본 정보 ━━
    st.subheader("👤 기본 정보")
    age = st.number_input("현재 나이", 15, 100, 35, key="age")
    gender_idx = st.session_state.get("gender_sel", 0)
    gender = st.selectbox("성별", ["남성","여성"], index=gender_idx, key="gender_box")
    gender_key = "male" if gender=="남성" else "female"
    retire_age = st.number_input("은퇴 나이", 30, 100, 60, key="retire")
    life_exp = st.number_input("기대 수명 (한국 평균 83.7세)", 60, 120, 85, key="life")
    inflation_rate = st.number_input("연간 물가상승률 (%)", 0.0, 15.0, 2.5, 0.1, key="infl",
    ...
    if is_simple:
        st.subheader("💰 자산 & 수입")
        total_savings = amt("총 모아놓은 자금 (만원)","ts",10000,1000,"예금+주식+부동산 등")
        monthly_income = amt_s("월 수입 — 세후 (만원)","mi",400)
        st.subheader("💸 지출")
        monthly_expense = amt_s("월 평균 지출 (만원)","me",250)
        ...
        st.subheader("🏦 보유 자산")
        st.markdown("**예금·적금**")
        deposit_amount = amt("예금 (만원)","da",3000,500,f"평균 금리 {DEP_RATE}%")
        deposit_rate = st.number_input("예금 수익률 (%)",0.0,20.0,DEP_RATE,0.1,key="dr")
        st.markdown("**주식·펀드·ETF**")
        stock_amount = amt("주식 (만원)","sa",4000,500)
        stock_return = st.number_input("기대 수익률 (%)",-20.0,30.0,7.0,0.5,key="sr")
        st.markdown("**부동산**")
        real_estate = amt("부동산 시가 (만원)","rea",30000,5000)
        re_return = st.number_input("부동산 상승률 (%)",-10.0,20.0,3.0,0.5,key="rr")
        other_assets = amt("기타 자산 (만원)","oa",0,500,"보험, 금 등")
        st.markdown("<hr class='section-divider'>", unsafe_allow_html=True)
        st.subheader("📈 수입")
        salary = amt_s("월 급여 — 세후 (만원)","sal",400)
        side_income = amt_s("부수입 (만원/월)","si",0)
        pension = amt_s("국민연금 예상 (만원/월)","pen",80,5)
        pension_start = st.number_input("연금 시작 나이",55,80,65,key="ps")
        st.markdown("<hr class='section-divider'>", unsafe_allow_html=True)
        st.subheader("💸 지출")
        st.info(f"📌 물가상승률 {inflation_rate}%: 변동비 100% · 고정비 50%")
        fixed_cost = amt_s("월 고정비 (만원) — 주거·보험·교육","fc",120)
        variable_cost = amt_s("월 변동비 (만원) — 식비·교통·여가","vc",80)
        savings_rate = st.number_input("저축률 (%)",0.0,100.0,30.0,5.0,key="svr")
        st.markdown("<hr class='section-divider'>", unsafe_allow_html=True)
        st.subheader("🏠 대출")
        num_loans = st.number_input("대출 건수",0,5,1,key="nl")
        loans = []
        for i in range(int(num_loans)):
            with st.expander(f"대출 {i+1}", expanded=(i==0)):
                la = amt(f"잔액 (만원)",f"la{i}",5000,1000)
                lr = st.number_input(f"이자율 (%)",0.0,20.0,LOAN_RATE,0.1,key=f"lr{i}")
                ly = st.number_input(f"남은 기간 (년)",1,40,20,key=f"ly{i}")
    ...
    if st.button("🚀 시뮬레이션 실행", use_container_width=True, type="primary"):
        ...
        # 결과를 session_state에 캐싱 (다운로드 시 초기화 방지)
    ...
    # ━━ 결과 표시 (session_state에서 읽기 → 다운로드해도 유지) ━━
    ...
    sign="+" if diff>=0 else ""; emoji="📈" if diff>=0 else "📉"
    st.markdown(f"""<div style="background:{'rgba(99,102,241,0.06)' if dark else 'rgba(79,70,229,0.04)'};
    ...
    f1,f2 = st.columns(2)
    with f1:
        if fire["years_left"]==0: st.metric("🔥 FIRE","달성!",f"목표: {fmt_krw(fire['target'])}")
        elif fire["years_left"]<0: st.metric("🔥 FIRE","도달 불가","저축액 부족")
        else: st.metric("🔥 FIRE까지",f"{fire['years_left']}년",f"진행률 {fire['progress']}%")
    with f2:
        if safe and safe["safe_monthly"]>0:
            st.metric("💳 안전 인출",f"{safe['safe_monthly']:,}만원/월",f"은퇴 후 {safe['years']}년간")
        else: st.metric("💳 안전 인출","계산 불가","은퇴 시 자산 부족")
    ...
    st.plotly_chart(chart_scenarios(base,s2,s3,params["retire_age"],dark),use_container_width=True,config=pcfg)
    ...
    # ━━ 다운로드 (session_state에 캐싱된 데이터 사용 → 초기화 안 됨) ━━
    ...
    if "pdf_cache" not in st.session_state:
        try:
            chart_imgs = generate_chart_images_for_pdf(
                ...
                base_df=base, s2_df=s2, s3_df=s3,
                opt_df=opt, pess_df=pess)
    ...
    st.code(f"?d={encoded}", language=None)
