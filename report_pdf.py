...
# 한글 CID 폰트 (시스템 폰트 불필요)
pdfmetrics.registerFont(UnicodeCIDFont("HYGothic-Medium"))
FONT = "HYGothic-Medium"

INDIGO = HexColor("#4338ca"); GREEN = HexColor("#059669")
RED = HexColor("#dc2626"); GRAY = HexColor("#64748b")
LIGHT = HexColor("#f1f5f9"); WHITE = HexColor("#ffffff")
BORDER = HexColor("#cbd5e1"); DARK = HexColor("#1e293b")

S_TITLE = ParagraphStyle("t", fontName=FONT, fontSize=18, leading=26,
                          textColor=INDIGO, alignment=TA_CENTER, spaceAfter=3*mm)
S_SUB   = ParagraphStyle("s", fontName=FONT, fontSize=9, leading=13,
                          textColor=GRAY, alignment=TA_CENTER, spaceAfter=6*mm)
S_H2    = ParagraphStyle("h", fontName=FONT, fontSize=12, leading=17,
                          textColor=INDIGO, spaceBefore=6*mm, spaceAfter=3*mm)
S_BODY  = ParagraphStyle("b", fontName=FONT, fontSize=9, leading=14,
                          textColor=DARK, spaceAfter=2*mm)
S_FOOT  = ParagraphStyle("f", fontName=FONT, fontSize=7.5, leading=11,
                          textColor=GRAY, alignment=TA_CENTER, spaceBefore=4*mm)
...
def _add_chart_image(elements, img_bytes, width_mm=160, label=""):
    """차트 이미지를 elements에 추가."""
    if img_bytes:
        img = Image(io.BytesIO(img_bytes), width=width_mm*mm,
                    height=width_mm*mm*0.5)
        elements.append(img)
...
def generate_pdf_report(df, params, dep_info, peak_info, current_info,
                         chart_images=None):
    """
    chart_images: dict of {"main": bytes, "composition": bytes,
                           "pie": bytes, "cashflow": bytes,
                           "scenarios": bytes, "sensitivity": bytes}
    """
    ...
    mt = Table(mdata, colWidths=[27*mm,18*mm,38*mm,38*mm,38*mm])
    ...
    cs = _base_ts() + [("BACKGROUND",(0,1),(0,-1),LIGHT)]
    ct = Table(cd, colWidths=[42*mm,118*mm])
    ct.setStyle(TableStyle(cs)); el.append(ct)

    # ━━ 면책 ━━
    el.append(Spacer(1,4*mm))
    el.append(HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceAfter=2*mm))
    el.append(Paragraph(
        "본 보고서는 참고용이며 실제 투자 성과와 다를 수 있습니다. "
